...
function hazardLevelFromMag(mag: number): {
  label: string;
  color: string;
  bg: string;
} {
  ...
  if (mag >= 4)
    return { label: "MODERATE", color: "#422006", bg: "#fbbf24" };
  if (mag >= 3)
    return { label: "LOW-MODERATE", color: "#166534", bg: "#bbf7d0" };
...
function ArchitectureDiagram() {
  const sources = [
    ...
    { y: 468, label: "Anthropic API", sub: "AI Situation Report", dot: "#ec4899" },
  ];
  const adapters = [
    ...
    { y: 468, label: "generateBriefing()", dot: "#ec4899" },
  ];
  const cache = [
    ...
    { y: 308, label: "weather", sub: "on demand", dot: "#22c55e" },
    { y: 388, label: "geocodeResults", sub: "per query", dot: "#22c55e" },
    { y: 468, label: "briefing (text)", sub: "on demand", dot: "#22c55e" },
  ];
  const panels = [
    { y: 55, label: "Interactive Map (Leaflet)", sub: "Quake & road markers, radius circle" },
    { y: 130, label: "Map Layer Toggles", sub: "Show / hide layer groups" },
    { y: 205, label: "Live Feed", sub: "Latest events within radius" },
    { y: 280, label: "Weather Card", sub: "Temp · wind · rain probability" },
    { y: 355, label: "AI Situation Report Panel", sub: "NHC-style risk briefing" },
    { y: 435, label: "Volcanic Alert Levels", sub: "All NZ volcanoes · GeoNet" },
    { y: 510, label: "Area Summary + Hazard Badge", sub: "Aggregated stats · risk level" },
  ];
    ...
    <main className="max-w-5xl mx-auto p-6">
      ...
      <div className="bg-white rounded-2xl border p-4 overflow-x-auto shadow-sm">
        ...
        >
          ...
          {/* Column backgrounds */}
          <rect x="8" y="8" width="192" height="572" rx="10" fill="#fef2f2" stroke="#fecaca" strokeWidth="1" />
          <rect x="212" y="8" width="172" height="572" rx="10" fill="#eff6ff" stroke="#bfdbfe" strokeWidth="1" />
          <rect x="396" y="8" width="162" height="572" rx="10" fill="#f0fdf4" stroke="#bbf7d0" strokeWidth="1" />
          <rect x="570" y="8" width="362" height="572" rx="10" fill="#fefce8" stroke="#fde68a" strokeWidth="1" />
      ...
      <p className="text-xs text-slate-400 mt-3">
        All sources are open/free-access. Calls are made directly from the
        browser (CORS-compliant). The Anthropic AI call requires an API key —
        set <code className="bg-slate-100 rounded px-1">VITE_ANTHROPIC_API_KEY</code> in{" "}
        <code className="bg-slate-100 rounded px-1">.env.local</code> or paste it in the
...
export default function App() {
  ...
  useEffect(() => {
    if (!mapDivRef.current || mapRef.current || typeof L === "undefined")
      return;
  ...
  /* Fix: layer toggles actually control Leaflet layer group visibility */
  useEffect(() => {
  ...
  async function geocodeAddress(q: string) {
    ...
    try {
      ...
      const res = await fetch(url.toString(), {
        headers: {
          "Accept-Language": "en",
          "User-Agent": "NHC-Hazard-Dashboard/1.0",
        },
      });
  ...
  /* Auto-refresh quakes + initial load on mount */
  useEffect(() => {
    loadQuakes();
    const countId = setInterval(
      () => setQuakeCountdown((c) => (c <= 1 ? QUAKE_REFRESH_S : c - 1)),
      1000
    );
  ...
  const quakesFiltered = useMemo(
    () =>
      quakes.filter((q) => {
        ...
        return (
          distanceKm(mapCenter[0], mapCenter[1], lat, lon) <= radius &&
          q.properties.magnitude >= magMin
        );
      }),
  ...
  useEffect(() => {
    loadRoadEvents();
    const countId = setInterval(
      () => setRoadCountdown((c) => (c <= 1 ? ROAD_REFRESH_S : c - 1)),
      1000
    );
  ...
  const loadWeather = useCallback(
    async (lat = mapCenter[0], lon = mapCenter[1]) => {
      try {
        setLoadingWx(true);
        setErrorWx(null);
        ...
        const precipProb =
          data.hourly?.precipitation_probability?.[nowHour] ?? null;
        if (cw) {
          setWeather({
  ...
  const loadVolcanoes = useCallback(async () => {
    try {
      setLoadingVol(true);
      setErrorVol(null);
      const res = await fetch(GEONET_VOLCANO, {
        headers: { Accept: "application/vnd.geo+json;version=2" },
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const generateBriefing = useCallback(async () => {
    if (!apiKey.trim())
      return setErrorBriefing(
        "Enter your Anthropic API key above first."
      );
    setLoadingBriefing(true);
    setErrorBriefing(null);
    setBriefing(null);
    try {
      const latStr = `${Math.abs(mapCenter[0]).toFixed(3)}°${mapCenter[0] < 0 ? "S" : "N"}`;
      const lonStr = `${Math.abs(mapCenter[1]).toFixed(3)}°${mapCenter[1] > 0 ? "E" : "W"}`;
      const context = [
        `Location: ${latStr}, ${lonStr} (${radius} km radius)`,
        quakesFiltered.length > 0
          ? `Earthquakes in area: ${quakesFiltered.length}, largest M${maxMag.toFixed(1)} at ${quakesFiltered[0]?.properties.depth ?? "–"} km depth (${quakesFiltered[0]?.properties.locality ?? ""})`
          : "Earthquakes in area: none detected above threshold",
        `Road/highway events in area: ${roadsFiltered.length}`,
        weather
          ? `Current weather: ${wmoCodeToDescription(weather.weathercode)}, ${weather.temperature}°C, wind ${weather.windspeed} km/h, rain probability ${weather.precipProb ?? "–"}%`
          : "Weather: not loaded",
        activeVolcanoes.length > 0
          ? `Active volcanoes: ${activeVolcanoes.map((v) => `${v.properties.volcanoTitle} (Level ${v.properties.level} – ${v.properties.activity})`).join("; ")}`
          : "Volcanic alerts: all volcanoes at background (Level 0)",
      ].join("\n");
      ...
      const res = await fetch(ANTHROPIC_MESSAGES, {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 350,
          system:
            "You are a senior hazard analyst for New Zealand's Natural Hazards Commission (NHC / Toka Tū Ake). Write a concise 4–5 sentence situation report for emergency managers. Include: overall risk level (Low / Moderate / High / Critical) with justification, the most significant active hazards, any notable watchpoints for the next 6–12 hours, and recommended actions if warranted. Use plain, professional emergency management language. Do not use markdown formatting.",
          messages: [
            {
              role: "user",
              content: `Current monitoring data:\n${context}\n\nProvide the situation report.`,
            },
          ],
        }),
      });
      ...
      setErrorBriefing(e.message || "Failed to generate briefing");
    } finally {
  ...
  const recentFeedEvents = useMemo(() => {
    const eqItems = quakesFiltered.slice(0, 3).map((q) => ({
      tag: "EQ",
      tagBg: "bg-red-100",
    ...
    const all = [...eqItems, ...roadItems].slice(0, 5);
    if (all.length === 0)
      return [
        {
          tag: "–",
          ...
          title: "No events within radius — adjust radius or click Refresh All",
          time: "",
          detail: "",
        },
      ];
    return all;
  }, [quakesFiltered, roadsFiltered]);
  ...
  return (
    <div className="min-h-screen bg-slate-100">
          ...
          <div className="ml-auto flex items-center gap-2 relative flex-wrap">
            <div className="relative">
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && geocodeAddress(geocodeQuery)
                }
              />
            ...
            <button
              onClick={() => geocodeAddress(geocodeQuery)}
      ...
      {view === "architecture" ? (
        ...
        <main className="max-w-7xl mx-auto p-4 grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <section className="lg:col-span-8 flex flex-col gap-4">
              ...
              <div className="p-3 flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-32"
                  />
                  <span className="text-sm w-16 text-right font-medium text-slate-700">
                    {radius} km
                  </span>
                </div>
              </div>
            ...
            {/* Layer toggles — now actually control map layers */}
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              <p className="text-xs text-slate-400 mt-2.5">
                Earthquakes and Road Events are live layers. Flood,
                Landslide, and Community layers are planned — integrations
                with LINZ, OpenTopography, and a community API are in scope.
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                />
                <textarea
                  className="rounded-xl border px-3 py-2 text-sm sm:col-span-2 resize-none"
                  rows={2}
                  placeholder="Describe what you see — e.g. water over road, estimated depth, time started"
                />
                <div className="flex items-center justify-between sm:col-span-2">
                  <span className="text-xs text-slate-400">
                    Coordinates taken from current map centre.
                  </span>
          ...
          <aside className="lg:col-span-4 flex flex-col gap-4">
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[
                  {
                    label: "Quakes in radius",
                    val: quakesFiltered.length,
                  },
                  {
                    label: "Road events",
                    val: roadsFiltered.length,
                  },
                  {
                    label: "Active volcanoes",
                    val: activeVolcanoes.length,
                  },
                  {
                    label: "Map centre",
                    val: `${mapCenter[0].toFixed(2)}, ${mapCenter[1].toFixed(2)}`,
                  },
                ].map(({ label, val }) => (
            ...
            {/* Live Feed (populated from real filtered data) */}
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              <ul className="divide-y">
                {recentFeedEvents.map((ev, i) => (
                  <li key={i} className="py-2.5 flex items-start gap-2">
                    <span
                      className={`rounded px-1.5 py-0.5 text-xs font-bold shrink-0 ${ev.tagBg} ${ev.tagColor}`}
                    >
                      {ev.tag}
                    </span>
                    <div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              <input
                type="password"
                className="w-full rounded-lg border px-3 py-1.5 text-sm mb-2"
                placeholder="Anthropic API key (or set VITE_ANTHROPIC_API_KEY)"
                value={apiKey}
              ...
              <button
                ...
                className="w-full rounded-lg bg-purple-700 text-white py-1.5 text-sm font-medium disabled:opacity-50 mb-2"
              >
                {loadingBriefing
                  ? "Generating report…"
              ...
              {errorBriefing && (
                <p className="text-xs text-red-600 mb-2">{errorBriefing}</p>
              )}
              {briefing ? (
                <div className="text-sm text-slate-700 leading-relaxed bg-purple-50 rounded-lg p-3 border border-purple-100">
                  {briefing}
                </div>
              ) : (
                !loadingBriefing &&
                !errorBriefing && (
                  <p className="text-xs text-slate-400">
                    Load data then generate an NHC-style risk briefing.
                  </p>
                )
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              {lastWxRefresh && (
                <div className="text-xs text-slate-400 mb-1">
                  Updated {fmtTime(lastWxRefresh)}
                </div>
              )}
              {errorWx && (
                <div className="text-xs text-red-600">{errorWx}</div>
              )}
              {weather ? (
                <div className="text-sm space-y-1">
                  <div className="font-medium text-slate-700">
                    {wmoCodeToDescription(weather.weathercode)}
                  </div>
                  <div className="text-slate-600">
                    🌡 {weather.temperature}°C &nbsp;·&nbsp; 💨{" "}
                    {weather.windspeed} km/h @ {Math.round(weather.winddirection)}°
                  </div>
                  {weather.precipProb !== null && (
                    <div
                      className={`text-sm ${
                        weather.precipProb >= 70
                          ? "text-blue-700 font-medium"
                          : "text-slate-600"
                      }`}
                    >
                      🌧 Rain probability: {weather.precipProb}%
                      {weather.precipProb >= 70 && " ⚠ High"}
                    </div>
                  )}
                  <div className="text-xs text-slate-400">
                    Open-Meteo · {mapCenter[0].toFixed(3)},{" "}
                    {mapCenter[1].toFixed(3)}
                  </div>
                </div>
              ) : (
                !loadingWx && (
                  <p className="text-xs text-slate-400">
                    Click Refresh or move the map to load weather.
                  </p>
                )
              )}
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              {errorQuakes && (
                <div className="text-xs text-red-600 mb-2">{errorQuakes}</div>
              )}
              {quakesFiltered.length === 0 && !loadingQuakes && (
                <div className="text-xs text-slate-400">
                  No quakes ≥ M{magMin} within {radius} km.
                </div>
              )}
              <ul className="divide-y">
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              {errorVol && (
                <div className="text-xs text-red-600 mb-2">{errorVol}</div>
              )}
                  ...
                  >
                    <span
                      ...
                      style={{
                        ...
                        color:
                          v.properties.level <= 1 ? "#166534" : "#fff",
                      }}
                    >
                      {v.properties.level}
                    </span>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-slate-800">
                        {v.properties.volcanoTitle}
                      </div>
                      <div className="text-xs text-slate-500 truncate">
                        {v.properties.activity}
                      </div>
                    </div>
                    {v.properties.activityBulletinURL && (
                      <a
                        href={v.properties.activityBulletinURL}
                        target="_blank"
                        rel="noreferrer"
            ...
            <div className="bg-white rounded-2xl shadow-sm border p-4">
              ...
              {lastRoadRefresh && (
                <div className="text-xs text-slate-400 mb-2">
                  Updated {fmtTime(lastRoadRefresh)}
                </div>
              )}
              {errorRoad && (
                <div className="text-xs text-red-600 mb-2">{errorRoad}</div>
              )}
              {roadsFiltered.length === 0 && !loadingRoad && !errorRoad && (
                <div className="text-xs text-slate-400">
                  No road events within {radius} km.
                </div>
              )}
              <ul className="divide-y">
                {roadsFiltered
      ...
      <footer className="max-w-7xl mx-auto px-4 py-5 text-xs text-slate-400 border-t mt-2">
        ...
        <a className="underline" href="https://www.anthropic.com" target="_blank">Anthropic</a>.{" "}
